class ReportBuilder
  ...
  def self.build_report(params)
    ...
    report[:incomes] = Income.where('reporting_period >= ? AND reporting_period <= ?', start_date, end_date)
    report[:costs] = Cost.where('created_at >= ? AND created_at <= ?', start_date, end_date)
